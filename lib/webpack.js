...
const normalizeWebpackEntry = (provideredMap) => {
  const returnMap = {}
  if (provideredMap && typeof provideredMap === 'object') {
    for (const name in provideredMap) {
      if (typeof provideredMap[name] === 'string') {
        returnMap[name] = [provideredMap[name]]
      } else {
        returnMap[name] = provideredMap
      }
    }
  }
  return returnMap
...
module.exports = (buildConfig, otherOptions) => {
      ...
      rules: [
        ...
        },
        {
          ...
          use: createExtractCssPlugin([
            ...
            createdPostcssLoader,
            {
              loader: require.resolve(otherOptions.scssLoaderName || 'sass-loader'),
              options: otherOptions.scssLoaderOptions || {},
            },
          ]),
        },
        {
  ...
  webpackConfig.entry = normalizeWebpackEntry(webpackConfig.entry, otherOptions)
  ...
  if (otherOptions.isDevelopment) {
    ...
    if (otherOptions.liveReloadOptions && typeof otherOptions.liveReloadOptions === 'object') {
      ...
      for (const keyName in webpackConfig.entry) {
        if (Array.isArray(webpackConfig.entry[keyName])) {
          webpackConfig.entry[keyName].unshift('webpack-dev-server/client?/')
        }
      }
    }
  } else {
